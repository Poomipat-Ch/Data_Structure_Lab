...
data = {"Acton Town" : ["Ealing Common","South Ealing","Turnham Green","Chiswick Park"],
...
class Graph:
    def __init__(self):
        self.น้ำหนัก = dict()
        self.น้ำหนักที่น้อยที่สุด = float('inf')

    def add_path(self, เริ่ม, path):
        self.น้ำหนัก[เริ่ม] = self.น้ำหนัก.get(เริ่ม, []) + [path]
    
    def min_path(self, เริ่ม, ระยะห่าง, visited):
        น้ำหนักที่น้อย =  self.น้ำหนักที่น้อยที่สุด

        for สถานี in self.น้ำหนัก:
            if ระยะห่าง[สถานี][1] < น้ำหนักที่น้อย and สถานี not in visited:
                น้ำหนักที่น้อย = ระยะห่าง[สถานี][1]
                เริ่ม = สถานี
    ...
    def dijkstra(self, src):
        ...
        for vertex in self.น้ำหนัก:  # set every vertex weight to infinity
            dist[vertex] = [vertex, self.น้ำหนักที่น้อยที่สุด]
        dist[src] = [src, 0]

        for vertex in self.น้ำหนัก:  # find shortest_path of every vertex
            src = self.min_path(src, dist, visited)
            visited += [src]
            for path in self.น้ำหนัก[src]:
                v = path[0]
                w = path[1]
...
เริ่ม, สิ้นสุด = input('Enter : ').split(',')
กราฟ = Graph()
for สถานี in data:
    for เส้นทาง in data[สถานี]:
        กราฟ.add_path(สถานี, [เส้นทาง, 1])
print(f'From: {เริ่ม}')
print(f'To: {สิ้นสุด}')
print('Searching shortest route... This may take a while...\nSuggested Route: ')
print(กราฟ.shortest_path(เริ่ม, สิ้นสุด))